#!/usr/bin/env python3
"""
Dynamic scraper pipeline that loads scraper modules based on a config file.
"""
import json
import importlib
import time
...
# Create a simple article cache module if the utils one is problematic
def load_cache():
    """Load article cache from file."""
...
def generate_hash(article):
    """Generate a hash for an article based on URL and title."""
    url = article.get('url', '')
...
def load_config():
    """
    Load the scraper configuration from the JSON file.
    Returns a list of dictionaries with scraper configurations.
    """
...
def import_scraper_function(function_path):
    """
    Dynamically import a scraper function from its path.
    Example: 'scrapers.taipei_times.scrape' -> from scrapers.taipei_times import scrape
...
def scrape_all_dynamic():
    ...
    """
    logger.info("Starting dynamic scraper pipeline")
    ...
    # For testing, use some sample data if no configs are loaded
    if not scraper_configs:
        logger.warning("Using sample test articles since no scrapers were configured")
        return [
            {
                "title": "Test Article 1",
                "url": "https://example.com/test1",
                "source": "test_source",
                "text": "This is test article 1 for the dynamic scraper.",
            ...
            },
            {
                ...
                "source": "test_source",
                "text": "This is test article 2 for the dynamic scraper.",
                "scraped_at": datetime.now().isoformat()
            }
        ]
    ...
    # If no articles were found, return sample data for testing
    if not all_new_articles:
        logger.warning("No new articles found. Returning sample test data.")
        return [
            {
                "title": "Sample Article for Testing",
                "url": "https://example.com/sample",
                "source": "sample_source",
                "text": "This is a sample article for testing when no real articles are found.",
                "scraped_at": datetime.now().isoformat()
            },
            {
                ...
                "source": "sample_source",
                "text": "This is another sample article for testing.",
                "scraped_at": datetime.now().isoformat()
            }
        ]
