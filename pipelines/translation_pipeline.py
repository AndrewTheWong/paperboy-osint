...
def translate_articles(articles: List[Dict], 
                      ...
                      translate_field: str = 'title') -> List[Dict]:
    """
    ...
    Args:
        ...
        translate_field: Field to translate ('title' or other field in the article dict)
    ...
    for i in range(0, len(articles), batch_size):
        ...
        texts = [article.get(translate_field, "") for article in batch]
        
        # Use provided language or get from article
        if from_lang:
            langs = [from_lang] * len(batch)
        else:
            langs = [article.get('language', translator.detect_language(article.get(translate_field, ""))) for article in batch]
        
        # Translate texts
        translated_texts = translator.translate_batch(texts, langs)
        
        # Update articles with translations
        for j, (article, translated_text) in enumerate(zip(batch, translated_texts)):
            article_copy = article.copy()
            
            # Add translated text field
            translated_field = f"translated_{translate_field}"
